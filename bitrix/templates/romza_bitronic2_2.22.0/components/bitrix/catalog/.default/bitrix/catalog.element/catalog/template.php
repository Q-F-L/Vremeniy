...
if($GLOBALS['ELEMENT_META']['META_H1'])
{
    $productTitle = $GLOBALS['ELEMENT_META']['META_H1'];
}
...
<?	endif ?>
        ...
        <p>Артикул: <?=$arResult['PROPERTIES']['CML2_ARTICLE']['VALUE']?></p>
<?
endif;
...
?>
	...
	<div class="short-info-top actions">
		<div id="<?= $id ?>" class="info rating rating-w-comments" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
		<? $frame = $this->createFrame($id, false)->begin($compositeLoader); $frame->setAssetMode(\Bitrix\Main\Page\AssetMode::STANDARD) ?>
			...
			<div class="product-photos<?=($bCatchbuy?' has-timer':'')?>" id="photo-block">
				<div class="stickers">
					<?if (is_array($arResult['BRAND_LOGO']) && !empty($arResult['BRAND_LOGO']['IMG']['src'])):?>
                        <a href="<?=$arResult['BRAND_LOGO']['URL']?>" class="brand">
                            ...
                            <img title="<?=$arResult['BRAND_LOGO']['ALT']?>" class="lazy" data-original="<?=$arResult['BRAND_LOGO']['IMG']['src']?>" src="<?=ConsVar::showLoaderWithTemplatePath()?>" alt="<?=$arResult['BRAND_LOGO']['ALT']?>">
                        </a>
					<?endif?>
				...
				*/
					...
					<div id="<? echo $arItemIDs['SLIDER_CONT_ID']; ?>" style="height:100%; width: 100%">
						<div class="product-photo">
							<img
								...
								data-big-src="<?=$arResult['MORE_PHOTO'][0]['SRC']?>"
								alt="<?= $strAlt ?>"
								title="<?= $strTitle ?>"
								itemprop="image">
						</div>
								...
								<div class="thumbnails-frame active">
									...
									<?foreach($arResult['MORE_PHOTO'] as $arPhoto):?>
										<div class="thumb">
											<img
                                                class="lazy"
                                                data-original="<?=$arPhoto['SRC']?>"
                                                src="<?=ConsVar::showLoaderWithTemplatePath()?>"
												alt="<?= $strAlt ?>"
                                                title="<?= $strAlt ?>"
												data-med-src="<?=$arPhoto['SRC']?>"
												data-big-src="<?=$arPhoto['SRC']?>"
											>
										</div>
									<?endforeach?>
				...
				<button type="button" class="btn-silver view3d disabled">
					<span class="text">3D<?//=GetMessage('BITRONIC2_3DVIEW')?></span>
				</button>
				<?endif?>
<? if ($arParams['QUICK_VIEW']): ?>
					...
					<div class="buy-block-main<?if(!($bShowEdost && !$arParams['EDOST_PREVIEW'])):?> __slim<? endif ?>">
						<div class="buy-block-content">
							...
							<div class="product-main-photo">
								<img id="<?=$arItemIDs['PICT_FLY']?>" src="<?=$arResult['DETAIL_PICTURE']['SRC']//=CResizer2Resize::ResizeGD2($arResult['DETAIL_PICTURE']['SRC'], $arParams["RESIZER_SETS"]['RESIZER_DETAIL_FLY_BLOCK'])?>" alt="<?= $strAlt ?>" title="<?= $strTitle ?>">
							</div>
							<div class="move">
...
?>
            ...
            <?if ($arResult['CATALOG_SUBSCRIBE'] == 'Y' && $arParams['ELEMENT_EXIST'] == 'Y' && $availableClass == 'out-of-stock'):?>
                <button type="button" class="btn-big to-waitlist btn-silver"  data-product="<?= $arResult['ID'] ?>" data-toggle="modal" data-target="#modal_subscribe_product">
                ...
                <?if($bShowOneClick && !$availableOnRequest && !$bBuyProps):?>
                    <div class="one-click-wrap">
                        <button id="<?= $arItemIDs['BUY_ONECLICK'] ?>" type="button"
                                class="btn-big buy btn-main action one-click-buy <?= ($canBuy || !empty($arResult['OFFERS']) ? '' : ' disabled') ?>"
            ...
            <?endif?>
                                ...
                                <?if ($bExpandedStore && $bShowStore):?>
                                    <?if($availableClass == 'in-stock' || empty($availableClass) || $arResult['bOffers']):?>
                                        <div class="availability <?=empty($availableClass)?'':$availableClass?>" id="catalog_store_amount_div_<?=$availableStoresPostfix?>_<?=$arResult['ID']?>">
                                            <script type="text/javascript">
                                                ...
                                                function initExpandedStores(){
                                                    if (typeof RZB2_initCommonHandlers != 'undefined'){
                                                        RZB2_initCommonHandlers.GetStoreContent('#<?=$arItemIDs['AVAIBILITY_EXPANDED']?>',<?=$arResult['ID']?>,'<?=$availableStoresPostfix?>');     } else{
...
if ($arParams['QUICK_VIEW']) {
	echo '
						 ...
						 <div class="sclad_i">На нашем складе имеются детали с небольшими дефектами, в худшем состоянии по более низкой цене. Уточняйте наличие и цену у менеджера</div>
...
if ($bShowEdost && $arParams['EDOST_PREVIEW']):?>
					...
					<div class="buy-block-additional edost">
						...
						<script type="application/javascript">BX.ready(function(){var spinner = new RZB2.ajax.spinner($('#edost_catalogdelivery_inside'));spinner.Start();});</script>
					</div>
<?
endif;
				...
				<div class="combo-links">
					...
					<?if($arParams['USE_REVIEW'] == 'Y'):?>
						<a href="#comments" class="combo-link">
							<i class="flaticon-speech90"></i>
							<span class="text"><?= $arParams['TITLE_TAB_REVIEWS'] ?: GetMessage('BITRONIC2_REVIEWS') ?></span><?// TODO <sup></sup>?>
						</a>
					<?endif?>
					<?if($bShowVideo):?>
					...
					<?if($bShowStore && $bTabsStore):?>
                        <?if($availableClass == 'in-stock' || empty($availableClass) || $arResult['bOffers']):?>
                            <a href="#catalog_store_amount_div_<?=$availableStoresPostfix?>_<?=$arResult['ID']?>" class="combo-link tab-store">
                                <i class="flaticon-folded11"></i>
					...
					<?endif?>
                    <?if($bReviewsItem): ?>
                        <a href="#review" class="combo-link">
                            <i class="flaticon-folded11"></i>
							...
							<div class="combo-target-content">
								...
								<div class="video">
									...
									<?else:
										...
										<iframe src="//www.youtube.com/embed/<?=$matches[2]?>" allowfullscreen></iframe>
									<?endif;?>
								</div><!-- /.video -->
								<?endforeach;?>
							</div>
...
<?= $arParams['DETAIL_INFO_MODE'] == 'tabs' ? ' tabs' : '' ?> <?=empty($availableClass)?'':$availableClass?>" >
                                <div class="combo-header">
                                    <i class="flaticon-folded11"></i>
                                    <span class="text"><?=$arParams['TITLE_TAB_STORES'] ? : GetMessage('MARKETS')?></span><?//:TODO<sup>3</sup>?>
                                </div>
                                <div class="combo-target-content" id="catalog_store_amount_div_<?=$availableStoresPostfix?>_<?=$arResult['ID']?>">
                                    <script type="text/javascript">
                                        require(['init/initMaps'], function(){
                                            b2.init.maps();
                                        });
                                        function initTabStores(){
                                            if (typeof RZB2_initCommonHandlers != 'undefined' && typeof RZB2_initCommonHandlers.initTabStores == 'undefined'){
                                                ...
                                                RZB2_initCommonHandlers.initTabStores = true;} else{
