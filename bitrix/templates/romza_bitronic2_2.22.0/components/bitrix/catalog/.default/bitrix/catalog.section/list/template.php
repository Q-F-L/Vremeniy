...
<?foreach($arResult['ITEMS'] as $arItem):
    ...
    $arItem['DETAIL_PICTURE']['SRC'] = $arItem['DETAIL_PICTURE']['SRC'] ? $arItem['DETAIL_PICTURE']['SRC'] : SITE_TEMPLATE_PATH.'/img/no-photo-product.gif';
    ...
    <?if ($count >= $showBannerOn && $arParams['BANNER_PLACE']):?>
        ...
        #BANNER_PLACE#
    <?endif?>
    <div class="catalog-item-wrap<?= ($arItem['VIP'] ? ' big-item' : '') ?>" id="<?=$arItemIDs['ID']?>">
		<div class="catalog-item list-item wow fadeIn">
			<div class="photo-wrap <?=!$arItem['SHOW_SLIDER'] ? ' no-thumbs' : ''?>">
				<div class="photo">
					<a title="<?= $imgTitle ?>" href="<?=$arItem['DETAIL_PAGE_URL']?>" class="<?=$arItemCLASSes['LINK']?>">
						<img class="lazy" data-original="<?=$arItem['DETAIL_PICTURE']['SRC']//$arItem['PICTURE_PRINT']['SRC']?>" src="<?=ConsVar::showLoaderWithTemplatePath()?>"  alt="<?=$arItem['PICTURE_PRINT']['ALT']?>" id="<?=$arItemIDs['PICT']?>" title="<?= $imgTitle ?>">
					</a><?
					if(!$bSkuExt):
						if($bCatchbuy):?>
							...
							<div class="slidee">
								<?foreach($arItem['MORE_PHOTO'] as $arPhoto):
									?><div class="photo-thumb">
										<img
                                            class="lazy" data-original="<?=$arItem['DETAIL_PICTURE']['SRC']//CResizer2Resize::ResizeGD2($arPhoto['SRC'], $arParams['RESIZER_SECTION_ICON'])?>" src="<?=ConsVar::showLoaderWithTemplatePath()?>"
							...
							<div class="photo-thumbs" id="<? echo $arItemIDs['SLIDER_CONT_OF_ID'].$arOffer['ID']; ?>" style="display:none">
								<div class="slidee">
									<?foreach($arOffer['MORE_PHOTO'] as $arPhoto):
										?><div class="photo-thumb">
											<img
                                                class="lazy" data-original="<?=$arItem['DETAIL_PICTURE']['SRC']//CResizer2Resize::ResizeGD2($arPhoto['SRC'], $arParams['RESIZER_SECTION_ICON'])?>"
                                                src="<?=ConsVar::showLoaderWithTemplatePath()?>"
												data-src="<?=$arItem['DETAIL_PICTURE']['SRC']//CResizer2Resize::ResizeGD2($arPhoto['SRC'], $arParams['RESIZER_SECTION_ICON'])?>"
